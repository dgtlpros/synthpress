    ...
    <LandingLayout user={user ? { email: user.email ?? "" } : null}>
      <section className="px-6 py-24">
        <div className="mx-auto max-w-3xl text-center">
          <span className="text-xs font-semibold uppercase tracking-wider text-brand-blue">About SynthPress</span>
          <h1 className="mt-3 text-4xl font-bold tracking-tight text-foreground sm:text-5xl">
            Publishing on autopilot, <span className="text-gradient-accent">without the slop</span>
          </h1>
          <p className="mt-6 text-lg leading-relaxed text-muted">
            SynthPress was built by operators who got tired of stitching together a half-dozen tools to keep their
            WordPress networks alive. We bundle generation, publishing, and syndication into one place — with the
            quality bar set at &ldquo;humans actually want to read this.&rdquo;
          </p>
        </div>
            ...
            <div className="grid gap-8 rounded-[calc(var(--sp-radius-xl)-1px)] bg-surface p-10 sm:grid-cols-3">
              {stats.map((s) => (
                <div key={s.label} className="text-center">
                  <div className="text-3xl font-bold text-gradient-accent sm:text-4xl">{s.value}</div>
        ...
        <div className="mx-auto mt-24 max-w-3xl">
          ...
          <div className="mt-6 space-y-5 text-base leading-relaxed text-muted">
            <p>
              We started SynthPress in late 2025 after spending years running content sites the hard way — managing
              writers, juggling editorial calendars, and watching margins shrink as AI-generated competitors flooded
              the search results.
            </p>
            <p>
              Most of the &ldquo;AI publishing&rdquo; tools we tried fell into two camps: thin wrappers that produced
              embarrassing copy, or enterprise platforms with onboarding flows that took weeks. Neither worked for the
              way we actually publish.
            </p>
            <p>
              So we built the thing we wanted: a single product that handles generation, publishing, and syndication
              with care taken at every step — and pricing that doesn&apos;t punish you for using it.
            </p>
          </div>
        </div>
      </section>
