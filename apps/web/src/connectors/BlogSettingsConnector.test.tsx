...
describe("BlogSettingsConnector", () => {
  it("renders blog name and control buttons", () => {
    ...
    expect(screen.getByRole("button", { name: /delete blog app/i })).toBeInTheDocument();
  });
  ...
  it("shows rename input when Rename is clicked", () => {
    ...
    expect(screen.getByRole("textbox", { name: /blog name/i })).toHaveValue("Tech Blog");
  ...
  it("hides rename input when Cancel is clicked", () => {
    render(
      <BlogSettingsConnector teamId="t1" projectId="p1" blogId="b1" blogName="Tech Blog" />,
    );
    fireEvent.click(screen.getByRole("button", { name: /rename/i }));
    fireEvent.click(screen.getByRole("button", { name: /cancel/i }));
    expect(screen.queryByRole("textbox", { name: /blog name/i })).not.toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "Tech Blog" })).toBeInTheDocument();
  });
  ...
  it("opens DeleteConfirmModal when Delete blog app is clicked", () => {
    ...
    expect(screen.getByRole("heading", { name: /delete blog app/i })).toBeInTheDocument();
  });
  ...
  it("calls deleteBlog when typed name matches and confirmed", () => {
    ...
    const deleteDialog = dialogs.find((d) => d.textContent?.includes("Delete blog app"));
    ...
    const confirmBtn = Array.from(deleteDialog!.querySelectorAll("button")).find(
      (b) => b.textContent?.includes("Delete blog app"),
    )!;
  ...
  it("shows renameError when present", () => {
    ...
    render(
      <BlogSettingsConnector teamId="t1" projectId="p1" blogId="b1" blogName="X" />,
    );
  ...
  it("closes DeleteConfirmModal when Cancel is clicked inside it", () => {
    ...
    expect(screen.queryByRole("heading", { name: /delete blog app/i })).not.toBeInTheDocument();
  });
  ...
  it("keeps rename input open when renameError is set (branch at handleRename)", () => {
    ...
    expect(screen.getByRole("textbox", { name: /blog name/i })).toBeInTheDocument();
  });
});
