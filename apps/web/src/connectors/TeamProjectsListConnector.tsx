...
import { ProjectsListToolbar, type ProjectsSortKey } from "@/components/molecules/ProjectsListToolbar";
...
export function TeamProjectsListConnector({ teamId, projects }: TeamProjectsListConnectorProps) {
  const [searchQuery, setSearchQuery] = useState("");
  const [sortKey, setSortKey] = useState<ProjectsSortKey>("name-asc");

  const filtered = useMemo(() => {
    const q = searchQuery.trim().toLowerCase();
    let rows = projects.map((p) => ({ id: p.id, name: p.name, created_at: p.created_at }));
    if (q) {
