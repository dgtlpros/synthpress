...
describe("ProjectOverviewConnector", () => {
  ...
  it("shows Delete project button for owner", () => {
    render(<ProjectOverviewConnector {...defaultProps} currentUserRole="owner" />);
    expect(screen.getByRole("button", { name: /delete project/i })).toBeInTheDocument();
  });

  it("hides Delete project button for member", () => {
    render(<ProjectOverviewConnector {...defaultProps} currentUserRole="member" />);
    expect(screen.queryByRole("button", { name: /delete project/i })).not.toBeInTheDocument();
  });

  it("hides Delete project button for admin", () => {
    render(<ProjectOverviewConnector {...defaultProps} currentUserRole="admin" />);
    expect(screen.getByRole("button", { name: /delete project/i })).toBeInTheDocument();
  });
  ...
  it("closes delete modal on Cancel", () => {
    ...
    expect(screen.queryByRole("heading", { name: /delete project/i })).not.toBeInTheDocument();
  });

  it("calls deleteProject when typed name matches and confirmed", async () => {
    mockDeleteProject.mockResolvedValue({ data: { redirect: "/teams/t1/projects" }, error: null });
    ...
    fireEvent.click(screen.getAllByRole("button", { name: /delete project/i }).find(
      (b) => b.closest("dialog"),
    )!);

    await vi.waitFor(() => expect(mockDeleteProject).toHaveBeenCalledWith("t1", "p1"));
  ...
  it("renders installed blogs as list items", () => {
    const blogs = [
      {
        id: "b1", name: "Tech Blog", slug: "tech-blog", project_id: "p1",
        wp_url: "https://wp.example.com", wp_username: "admin", is_active: true,
        articles_per_day: 2, niche: "Tech", keywords: [], ai_prompt_template: "",
        schedule_cron: "0 9 * * *", created_at: "2026-01-01", updated_at: "2026-01-01",
      },
    ] as never[];
  ...
  it("shows error in settings modal on save failure", async () => {
    mockUpdateProjectSettings.mockResolvedValue({ data: null, error: "Name taken" });

    render(<ProjectOverviewConnector {...defaultProps} />);
    fireEvent.click(screen.getByRole("button", { name: /settings/i }));
    fireEvent.click(screen.getByRole("button", { name: /^save$/i }));

    await vi.waitFor(() => expect(screen.getByText("Name taken")).toBeInTheDocument());
  });

  it("shows delete error when deleteProject fails", async () => {
    mockDeleteProject.mockResolvedValue({ data: null, error: "Permission denied" });

    render(<ProjectOverviewConnector {...defaultProps} />);
    fireEvent.click(screen.getByRole("button", { name: /delete project/i }));

    const dialogs = screen.getAllByRole("dialog", { hidden: true });
    const deleteDialog = dialogs.find((d) => d.textContent?.includes("Delete project"));
    fireEvent.change(deleteDialog!.querySelector("input")!, { target: { value: "Website" } });
    const confirmBtn = Array.from(deleteDialog!.querySelectorAll("button")).find(
      (b) => b.textContent?.includes("Delete project"),
    )!;
    fireEvent.click(confirmBtn);

    await vi.waitFor(() => expect(screen.getByText("Permission denied")).toBeInTheDocument());
  });
  ...
  it("closes CreateAppChoiceModal via onClose", () => {
    ...
    const dialog = screen.getByRole("heading", { name: /create app/i }).closest("dialog")!;
    fireEvent(dialog, new Event("cancel", { cancelable: true }));
    expect(screen.queryByRole("heading", { name: /create app/i })).not.toBeInTheDocument();
  });
