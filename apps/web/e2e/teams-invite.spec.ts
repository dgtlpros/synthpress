...
test("unauthenticated visit to /teams/invite/<token> redirects to /login", async ({ page }) => {
  await page.goto("/teams/invite/some-test-token");
  await expect(page).toHaveURL(/\/login/);
});

test("unauthenticated visit to /teams/<id>/settings redirects to /login", async ({ page }) => {
  await page.goto("/teams/00000000-0000-0000-0000-000000000000/settings");
  await expect(page).toHaveURL(/\/login/);
});

test("unauthenticated visit to /teams/<id>/usage redirects to /login", async ({ page }) => {
