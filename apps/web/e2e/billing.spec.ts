...
test("pricing CTAs route unauthenticated visitors to signup", async ({ page }) => {
  await page.goto("/pricing");
  const cta = page.getByRole("link", { name: "Get Started" }).first();
  await expect(cta).toHaveAttribute("href", /\/signup/);
});

test("unauthenticated user is redirected from /account/billing to /login", async ({ page }) => {
...
test("/api/webhooks/stripe rejects requests without a signature", async ({ request }) => {
